...
export const BentoGridItem = ({
  ...
  useEffect(() => {
    setIsClient(true); // Ensures Lottie only loads on the client
  }, []);
  // useEffect(() => {
  //   return () => {
  //     if (Lottie) {
  ...
  const handleCopy = () => {
    const text = 'stephengmre@gmail.com';
    ...
    >
      ...
      <div className={`${id === 6 && 'flex justify-center'} h-full`}>
        ...
        >
          ...
          <div className='font-sans font-extralight md:max-w-32 md:text-xs lg:text-base text-sm text-gray-600 dark:text-[#C1C2D3] z-10'>
            {description}
          </div>
          {/* add text-3xl max-w-96 , remove text-neutral-600 dark:text-neutral-300*/}
          {/* remove mb-2 mt-2 */}
          <div
            className={`font-sans text-lg lg:text-3xl max-w-96 font-bold text-gray-800 dark:text-white  z-10`}
          >
            {title}
          </div>
              ...
              <div className='flex flex-col gap-3 md:gap-3 lg:gap-4'>
                {leftLists.map((item, i) => (
                  <span
                    ...
                    lg:opacity-100 rounded-lg text-center bg-gray-100 dark:bg-[#10132E]'
                  >
                    {item}
                  </span>
                ))}
